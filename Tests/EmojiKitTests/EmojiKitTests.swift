  ...
  let fetcher = EmojiDataLoader(
    httpClient: StubHTTPClient(
      responses: [
        ...
        release.unicodeBaseURL.appending(path: "emoji-sequences.txt").absoluteString: .failure(404),
        release.unicodeBaseURL.appending(path: "emoji-zwj-sequences.txt").absoluteString: .success(
          "zwj"
        ),
...
@Test func fetchLocalizationSourcesAllowsDerivedOnlyLocale() async throws {
  ...
  let fetcher = EmojiDataLoader(
    httpClient: StubHTTPClient(
      responses: [
        release.cldrBaseURL.appending(path: "cldr-annotations-full/annotations/aa/annotations.json")
  ...
  let bundle = GeneratedBundle(
    ...
    localizations: [
      "en": GeneratedLocalizationFile(
        localeIdentifier: "en",
        entries: [
          "1F600": GeneratedLocalizationEntry(name: "grinning face", searchTokens: ["face", "grin"])
        ]
      )
    ]
  )
